/**
 * Search metadata for the 21 `/solution/<slug>` pages.
 *
 * `lib/ces/solutions.ts` is GENERATED by `scripts/extract-solutions.mjs` and carries the
 * originals' own `<title>`/`<meta description>` — a bare industry noun ("Realtor") and, on
 * ten of the twenty-one pages, the same copy-pasted "IT and operations teams" description.
 * Rewriting those in the generated file would be undone by the next extraction run, so the
 ...
 */
export type SolutionSeo = { title: string; description: string };

export const solutionSeo: Record<string, SolutionSeo> = {
  accountants: {
...
export function getSolutionSeo(slug: string): SolutionSeo | undefined {
